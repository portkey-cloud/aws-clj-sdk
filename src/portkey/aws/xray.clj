...
(clojure.core/defn get-trace-graph "Retrieves a service graph for one or more specific trace IDs.\n" ([get-trace-graph-request] (portkey.aws/-rest-json-call portkey.aws.xray/endpoints "POST" "/TraceGraph" get-trace-graph-request :portkey.aws.xray/get-trace-graph-request {:payload nil, :move {}, :headers {}, :uri {}, :querystring {}} nil :portkey.aws.xray/get-trace-graph-result {"InvalidRequestException" :portkey.aws.xray/invalid-request-exception, "ThrottledException" :portkey.aws.xray/throttled-exception})))
...
(clojure.core/defn put-telemetry-records "Used by the AWS X-Ray daemon to upload telemetry.\n" ([put-telemetry-records-request] (portkey.aws/-rest-json-call portkey.aws.xray/endpoints "POST" "/TelemetryRecords" put-telemetry-records-request :portkey.aws.xray/put-telemetry-records-request {:payload nil, :move {}, :headers {}, :uri {}, :querystring {}} nil :portkey.aws.xray/put-telemetry-records-result {"InvalidRequestException" :portkey.aws.xray/invalid-request-exception, "ThrottledException" :portkey.aws.xray/throttled-exception})))
...
(clojure.core/defn put-trace-segments "Uploads segment documents to AWS X-Ray. The X-Ray SDK generates segment documents and sends them to the X-Ray daemon, which uploads them in batches. A segment document can be a completed segment, an in-progress segment, or an array of subsegments.\n Segments must include the following fields. For the full segment document schema, see AWS X-Ray Segment Documents (http://docs.aws.amazon.com/xray/latest/devguide/xray-api-segmentdocuments.html) in the AWS X-Ray Developer Guide.\n  Required Segment Document Fields \n  \n *  name - The name of the service that handled the request.\n  \n *  id - A 64-bit identifier for the segment, unique among segments in the same trace, in 16 hexadecimal digits.\n  \n *  trace_id - A unique identifier that connects all segments and subsegments originating from a single client request.\n  \n *  start_time - Time the segment or subsegment was created, in floating point seconds in epoch time, accurate to milliseconds. For example, 1480615200.010 or 1.480615200010E9.\n  \n *  end_time - Time the segment or subsegment was closed. For example, 1480615200.090 or 1.480615200090E9. Specify either an end_time or in_progress.\n  \n *  in_progress - Set to true instead of specifying an end_time to record that a segment has been started, but is not complete. Send an in progress segment when your application receives a request that will take a long time to serve, to trace the fact that the request was received. When the response is sent, send the complete segment to overwrite the in-progress segment.\n   A trace_id consists of three numbers separated by hyphens. For example, 1-58406520-a006649127e371903a2de979. This includes:\n  Trace ID Format \n  \n * The version number, i.e. 1.\n  \n * The time of the original request, in Unix epoch time, in 8 hexadecimal digits. For example, 10:00AM December 2nd, 2016 PST in epoch time is 1480615200 seconds, or 58406520 in hexadecimal.\n  \n * A 96-bit identifier for the trace, globally unique, in 24 hexadecimal digits.\n  " ([put-trace-segments-request] (portkey.aws/-rest-json-call portkey.aws.xray/endpoints "POST" "/TraceSegments" put-trace-segments-request :portkey.aws.xray/put-trace-segments-request {:payload nil, :move {}, :headers {}, :uri {}, :querystring {}} nil :portkey.aws.xray/put-trace-segments-result {"InvalidRequestException" :portkey.aws.xray/invalid-request-exception, "ThrottledException" :portkey.aws.xray/throttled-exception})))
